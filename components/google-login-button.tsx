...
export function GoogleLoginButton({
  ...
  const handleGoogleSuccess = async (tokenResponse: any) => {
    ...
    try {
      ...
      console.log("Google user info received:", { email: userInfo.email })
  ...
  // Initialize Google login with explicit redirect URI
  const googleLogin = useGoogleLogin({
    onSuccess: handleGoogleSuccess,
    onError: (error) => {
      console.error("Google login error:", error)
      console.log("Current origin at error time:", window.location.origin)
      console.log("Full URL at error time:", window.location.href)
      setGoogleLoading(false)
      toast({
        title: "Google Sign-in Error",
        description: "Failed to authenticate with Google. Check console for details.",
        variant: "destructive",
      })
    },
    scope: "email profile",
    flow: "implicit",
    // Removed unsupported ux_mode property
  })

  const handleClick = () => {
    setGoogleLoading(true)
    console.log("Starting Google login with origin:", window.location.origin)
    googleLogin()
  }
